...
class ParallelComponentExecutor:
    """Execute multiple components in parallel with file conflict handling.

    Used by the DAG scheduler to execute batches of components.
    """
    ...
    def execute_batch(
        ...
        workflow_id: str,
        on_complete: Any | None = None,  # Callable[[str, ComponentStatus, str], None]
    ) -> dict[str, Any]:
        """Execute a batch of components in parallel.

        Args:
            components: List of Component objects to execute
            workflow_id: Parent workflow/feature ID
            on_complete: Callback(component_id, status, output_or_error)

        Returns:
            Dict mapping component_id to execution result
        """
        ...
        if len(components) == 1:
            ...
            comp = components[0]
            try:
                result = self._execute_single(comp, workflow_id)
                results[comp.id] = {"success": True, "output": result}
                if on_complete:
                    on_complete(comp.id, ComponentStatus.COMPLETED, str(result))
            except Exception as e:
                results[comp.id] = {"success": False, "error": str(e)}
                if on_complete:
                    on_complete(comp.id, ComponentStatus.FAILED, str(e))
            return results

        # Execute batch in parallel
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(self._execute_single, comp, workflow_id): comp
    ...
    def _execute_single(self, component: Any, workflow_id: str) -> Any:
        """Execute a single component using the engine."""
